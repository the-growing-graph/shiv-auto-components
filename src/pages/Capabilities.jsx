...
import { Cpu, Ruler, Wrench, Layers, Cog, ShieldCheck, ScanSearch, Factory, CheckCircle2 } from "lucide-react";
...
const CAPS = [
  {
    ...
    process: ["Tool selection & CAM programming", "First-off inspection", "Serial run with in-process gauging"],
    machines: ["Doosan Turning (Ø400×1000)", "Haas VMC (1200×600×600)", "5-axis DMG Mori"],
  ...
  },
  {
    ...
    process: ["DFM & Cpk study", "Prototype validation (PPAP L3)", "Mass production run"],
  ...
  },
  {
    ...
    process: ["Nesting optimization", "Laser cut → bend → weld", "Powder coat & assembly"],
    machines: ["6kW Fiber Laser (3×1.5m)", "Amada 300T Press Brake", "ABB Robotic Weld Cells"],
  ...
  },
  {
    ...
    process: ["Line balancing", "Torque tool & sequence control", "End-of-line testing"],
    machines: ["DC Nutrunners", "Vision inspection stations", "Leak & function test rigs"],
  ...
  },
  {
    ...
    process: ["Concept in CAD (SolidWorks)", "Simulation & DFM", "Machining & try-out"],
  ...
  },
  {
    ...
    process: ["Chemical pre-treatment", "Coating (electrostatic / dip)", "Curing & inspection"],
  ...
  },
  {
    ...
    process: ["Incoming material inspection", "In-process SPC", "Final Cpk & PPAP"],
  ...
  },
  {
    ...
    process: ["Requirement study", "Prototype in weeks", "Scale to serial production"],
    ...
    <div data-testid="capabilities-page" className="pb-24">
      <section className="py-24 lg:py-32 bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold text-[#0F2B46] max-w-4xl leading-[1.02] tracking-tight">
            Eight capabilities. One integrated shop floor.
          </h1>
          <p className="mt-6 max-w-2xl text-[#6B7280] text-lg leading-relaxed">
            From incoming raw material to finished packed dispatch — everything is co-located, controlled, and measured.
          </p>
        </div>
      </section>
      ...
      <section className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-24">
          {CAPS.map((c, idx) => (
            <Reveal key={c.title}>
              <div className={`grid lg:grid-cols-2 gap-12 items-center ${idx % 2 ? "lg:[&>div:first-child]:order-2" : ""}`}>
                <div>
                  <c.icon className="w-10 h-10 text-[#2563EB]" strokeWidth={1.75} />
                  <h2 className="mt-6 font-display font-bold text-3xl sm:text-4xl text-[#0F2B46] tracking-tight">
                    {c.title}
                  </h2>
                  <p className="mt-4 text-[#6B7280] text-lg leading-relaxed">{c.overview}</p>

                  <div className="mt-8 grid sm:grid-cols-2 gap-6">
                    <div>
                      <div className="font-eyebrow text-[#0F2B46] mb-3">Process</div>
                      <ul className="space-y-2">
                        {c.process.map((p) => (
                          <li key={p} className="flex items-start gap-2 text-sm text-[#0F2B46]">
                    ...
                    <div>
                      <div className="font-eyebrow text-[#0F2B46] mb-3">Machines</div>
                      <ul className="space-y-2">
                        {c.machines.map((m) => (
                          <li key={m} className="text-sm text-[#0F2B46]">• {m}</li>
                        ))}
                      </ul>
                    </div>
                  </div>
                  ...
                  <div className="mt-6 grid sm:grid-cols-2 gap-6">
                    <div>
                      <div className="font-eyebrow text-[#0F2B46] mb-2">Applications</div>
                      <p className="text-sm text-[#6B7280]">{c.applications}</p>
                    </div>
                    <div>
                      <div className="font-eyebrow text-[#0F2B46] mb-2">Capacity</div>
                  ...
                  <div className="mt-6 flex flex-wrap gap-2">
                    {c.industries.map((i) => (
                      <span key={i} className="text-xs font-eyebrow px-3 py-1 border border-[#2563EB] text-[#2563EB] rounded-sm">{i}</span>
                    ))}
                  </div>
                </div>
